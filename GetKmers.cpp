...
void query_kmers_in_read( char *read, char *qual, int kmerLength, double alpha, KmerCode &kmerCode, StoreCML *kmerCounters, size_t* kcount_seen )
{
	int i ;
	double p ;
	double factor = 1 ;
...
uint64_t CountKmers( char *read, char *qual, int kmerLength,
	KmerCode &kmerCode, StoreCML *kmers, int cutoff )
{
	int i ;
	...
	int count = kmers->IsInExact( kmerCode );
	if(count > cutoff) {
		total_count+=1;
	...
	{
		kmerCode.Append( read[i] ) ;
		count = kmers->IsInExact( kmerCode );
		if(count > cutoff) {
			total_count+=1;
			//kmers->decrease(kmerCode, count); 
		}
